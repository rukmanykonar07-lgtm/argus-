...
function load() {
  ...
  return { clients: [], adAccounts: [], dailyMetrics: [], searchVisibility: {}, nextIds: { client: 1, adAccount: 1, metric: 1 } };
}

let data = load();
// Back-compat: data files saved before searchVisibility existed won't have it
...
function buildTree(rows) {
  ...
  for (const m of rows) {
    if (!campaigns[m.campaign_name]) campaigns[m.campaign_name] = { agg: emptyAgg(), adsets: {}, platform: m.platform, ad_account_id: m.ad_account_id };
...
function getClientSummary(clientId, { days = 30, since, until, platform } = {}) {
  ...
  const campaigns = Object.entries(currTree.campaigns).map(([campName, camp]) => {
    ...
    return {
      ...
      insight: campInsight,
      searchVisibility: camp.platform === 'google' ? getSearchVisibility(camp.ad_account_id, campName) : null,
      adsets,
    };
  }).sort((a, b) => b.spend - a.spend);
  ...
  return {
    ...
    brandInsight, searchVisibilityAvg,
    deltas: {
