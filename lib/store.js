...
function load() {
  if (fs.existsSync(DATA_FILE)) {
    try {
      ...
      // Never silently discard real client data. A half-written file from
      // a crash, power loss, or a sync-conflict (plausible — this folder
      // can end up inside OneDrive/Dropbox on a client's laptop) used to
      // mean starting fresh with zero warning and zero way to recover.
      // Back the corrupted file up before touching anything.
      const backupPath = DATA_FILE.replace(/\.json$/, `.corrupted-${Date.now()}.json`);
      try {
        fs.copyFileSync(DATA_FILE, backupPath);
        console.error(`ARGUS Pulse: data file was corrupted (${e.message}). Backed it up to ${backupPath} and starting fresh — that file still has your old data if you need it recovered.`);
      } catch (backupErr) {
        console.error(`ARGUS Pulse: data file was corrupted (${e.message}) and a backup could not be made (${backupErr.message}). Starting fresh — the original corrupted file is still at ${DATA_FILE}.`);
      }
    }
  }
...
function save() {
  ...
  // This is the other half of the corruption fix in load() above: this
  // prevents it from happening, that one recovers if it happens anyway.
...
// Sets credentialsCorrupted: true (rather than throwing) if any stored
// token exists but fails to decrypt, so the caller (syncAccount) can
// surface that distinctly instead of it looking like "no token was ever
// set" — see the comment on decrypt() in lib/crypto.js for why that
// distinction matters.
function getAdAccount(id) {
  const a = data.adAccounts.find(a => a.id === Number(id));
  if (!a) return a;
  const result = { ...a, credentialsCorrupted: false };
  for (const field of ['access_token', 'developer_token', 'refresh_token', 'client_secret']) {
    try {
      result[field] = decrypt(a[field]);
    } catch (e) {
      if (e.isDecryptFailure) { result[field] = null; result.credentialsCorrupted = true; }
      else throw e;
    }
  }
  return result;
...
function markSynced(accountId, mode) {
  // Must look up the account directly in the array, NOT via getAdAccount()
  // — that returns a spread copy (`{ ...a }`) for safe external use, so
  // mutating fields on it never touched the real stored object. This was
  // silently no-op-ing every single sync since the function was first
  // written: last_synced_at and last_sync_mode never actually persisted,
  // verified empirically (wrote, called markSynced, read the file back,
  // field was still null) before writing this fix.
  const a = data.adAccounts.find(a => a.id === Number(accountId));
