...
function auditEntity(curr, searchVisibility) {
  ...
  // ponytail: flat spend threshold, not scaled per-client — a ₹10k/day
  // account and a ₹500/day account get the same bar here. Fine as a
  // first pass; switch to a per-client-relative threshold if this proves
  // noisy in practice.
  const MIN_SPEND_FOR_TRACKING_CHECK = 1000;
  if (curr.spend >= MIN_SPEND_FOR_TRACKING_CHECK && (curr.conversions || 0) === 0) {
    findings.push({
      title: 'Zero conversions despite real spend',
      explanation: `₹${curr.spend.toFixed(0)} spent with 0 conversions recorded. Worth checking the conversion pixel/tag is actually firing before assuming the campaign itself is the problem.`,
    });
  }
